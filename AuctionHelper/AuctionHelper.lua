...
function ah_proposal(bid, item_name, vol, price)
	...
	elseif bid == 'sell' then
		if not auction_box then print('AH Error: Click auction counter or use //ah to initialize sales.') return false end
		if not find_empty_slot() then print('AHPack Error: No Empty Slots Available.') return false end
		...
		if not index then print('AHPack Error: %s of %s not found in inventory.':format(single == 1 and 'Single' or 'Stack',item.en)) return false end
